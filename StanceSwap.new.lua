...
local function GetContains(stances, predicate)
	for stateId, stateName in ipairs(gStance) do
		for _, swapId in ipairs(stances) do
			if stateId == swapId and (event == nil or event(stateId)) then
...
local function GetReady(slots)
	...
	local check = slots[slotID];
	-- Проверка на наличие условия.
	if check == nil or check == slotID then
		-- Условия нет, считаем это выполнимым.
		return true;
	end
	-- ANI.HINT>
...
function StanceSwaped(inStances,inBefore,inAfter,outStance,outBefore,outAfter)
	...
	then
		...
		SetStance(inStances,
			function()
				for _,act in ipairs(inBefore) do
					SpellStopCasting()UseAction(act);
				end
			end,
			function()
				for _,act in ipairs(inAfter) do
					SpellStopCasting()UseAction(act);
				end
			end);
		...
		SetStance({outStance},
			function()
				for _,act in ipairs(outBefore) do
					SpellStopCasting()UseAction(act);
				end
			end,
			function()
				for _,act in ipairs(outAfter) do
					SpellStopCasting()UseAction(act);
				end
			end);
	end
